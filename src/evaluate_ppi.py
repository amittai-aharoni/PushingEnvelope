...
from src.utils.ppi_data_loader import load_protein_data
...
def evaluate(dataset, embedding_size, split="test"):
    ...
    model = LoadedModel.from_name(
        "boxsqel", f"data/PPI/{dataset}/boxsqel", embedding_size, device, best=True
    )
    ...
    ranks, top1, top10, top100, filtered_ranks, ftop1, ftop10, ftop100 = compute_ranks(
        model, test_data, device, filtering_dict, use_tqdm=True
    )
    ...
    standard_output = (
        f"Standard: {top10:.2f},{top100:.2f},"
        f"{np.mean(ranks):.2f},{np.median(ranks)},{rank_auc:.2f}\n"
    )
    filtered_output = (
        f"Filtered: {ftop10:.2f},{ftop100:.2f},"
        f"{np.mean(filtered_ranks):.2f},{np.median(filtered_ranks)},{frank_auc:.2f}"
    )

    output = standard_output + filtered_output
    print(output)
...
def compute_ranks(model, eval_data, device, filtering_dict=None, use_tqdm=False):
    top1 = top10 = top100 = ftop1 = ftop10 = ftop100 = 0.0
    ...
    r = eval_data[0, 1]
    # assume we use the same r everywhere
    assert (eval_data[:, 1] != r).sum() == 0
    ...
    for i in range_fun(num_batches):
        ...
        bumped_c_centers = (
            torch.tile(embeds, (current_batch_size, 1, 1)) + d_bumps[:, None, :]
        )
        bumped_d_centers = d_embeds[:, None, :] + torch.tile(
            bumps, (current_batch_size, 1, 1)
        )
        ...
        batch_ranks = torch.take_along_dim(
            index, batch_data[:, 0].reshape(-1, 1), dim=1
        ).flatten()
...
def load_filtering_dict(dataset, proteins, relations, device):
    ...
    for c, r, d in train_data:
        if r not in filtering_dict:
            filtering_dict[r] = torch.ones(
                (len(proteins), len(proteins)), requires_grad=False
            ).to(device)
