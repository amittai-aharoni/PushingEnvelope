...
from src.model.loaded_models import BoxELLoadedModel
...
def l2_side_regularizer(box, log_scale: bool = True):
    ...
    if not log_scale:
        ...
        return torch.mean(F.relu(min_x + delta_x - 1 + eps)) + F.relu(
            torch.norm(min_x, p=2) - 1
        )
...
class BoxEL(nn.Module):
    def __init__(self, device, class_, relationNum, embedding_dim):
        ...
        delta_embedding = self.init_concept_embedding(
            vocab_size, embedding_dim, delta_init_value
        )
        relation_embedding = self.init_concept_embedding(
            relationNum, embedding_dim, relation_init_value
        )
        scaling_embedding = self.init_concept_embedding(
            relationNum, embedding_dim, scaling_init_value
        )
    ...
    def inclusion_loss(self, boxes1, boxes2):
        log_intersection = torch.log(
            torch.clamp(self.volumes(self.intersection(boxes1, boxes2)), 1e-10, 1e4)
        )
    ...
    def nf1_loss(self, boxes1, boxes2):
        return self.inclusion_loss(boxes1, boxes2), l2_side_regularizer(
            boxes1, log_scale=True
        ) + l2_side_regularizer(boxes2, log_scale=True)

    def nf2_loss(self, boxes1, boxes2, boxes3):
        inter_box = self.intersection(boxes1, boxes2)
        return self.inclusion_loss(inter_box, boxes3), l2_side_regularizer(
            inter_box, log_scale=True
        ) + l2_side_regularizer(boxes1, log_scale=True) + l2_side_regularizer(
            boxes2, log_scale=True
        ) + l2_side_regularizer(
            boxes3, log_scale=True
        )
    ...
    def nf3_loss(self, boxes1, relation, scaling, boxes2):
        ...
        return self.inclusion_loss(trans_boxes, boxes2), l2_side_regularizer(
            trans_boxes, log_scale=True
        ) + l2_side_regularizer(boxes1, log_scale=True) + l2_side_regularizer(
            boxes2, log_scale=True
        )
    ...
    def nf4_loss(self, relation, scaling, boxes1, boxes2):
        ...
        trans_boxes = Box(trans_min, trans_max)
        #         log_trans_boxes = torch.log(
        #         torch.clamp(self.volumes(trans_boxes), 1e-10, 1e4))
        return self.inclusion_loss(trans_boxes, boxes2), l2_side_regularizer(
            trans_boxes, log_scale=True
        ) + l2_side_regularizer(boxes1, log_scale=True) + l2_side_regularizer(
            boxes2, log_scale=True
    ...
    def disjoint_loss(self, boxes1, boxes2):
        ...
        return torch.exp(log_intersection - union), l2_side_regularizer(
            boxes1, log_scale=True
        ) + l2_side_regularizer(boxes2, log_scale=True)
    ...
    def nf3_neg_loss(self, boxes1, relation, scaling, boxes2):
        ...
        #         trans_max = trans_min + torch.clamp((boxes1.max_embed -
        #         boxes1.min_embed)*(scaling + eps), 1e-10, 1e4)
        #         trans_boxes = Box(trans_min, trans_max)
        return 1 - self.inclusion_loss(trans_boxes, boxes2), l2_side_regularizer(
            trans_boxes, log_scale=True
        ) + l2_side_regularizer(boxes1, log_scale=True) + l2_side_regularizer(
            boxes2, log_scale=True
        )
    ...
    def save(self, folder, best=False):
        ...
        np.save(
            f"{folder}/min_embeds{suffix}.npy",
            self.min_embedding.detach().cpu().numpy(),
        )
        np.save(
            f"{folder}/delta_embeds{suffix}.npy",
            self.delta_embedding.detach().cpu().numpy(),
        )
        np.save(
            f"{folder}/rel_embeds{suffix}.npy",
            self.relation_embedding.detach().cpu().numpy(),
        )
        np.save(
            f"{folder}/scaling_embeds{suffix}.npy",
            self.scaling_embedding.detach().cpu().numpy(),
        )
