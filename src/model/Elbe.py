# Code adapted from
# https://github.com/bio-ontology-research-group/EL2Box_embedding.
...
from src.model.loaded_models import ElbeLoadedModel
...
class Elbe(nn.Module):
    ...
    def __init__(
        self, device, class_, relationNum, embedding_dim, margin=0, vis_loss=False
    ):
        ...
        self.classEmbeddingDict.weight.data /= torch.linalg.norm(
            self.classEmbeddingDict.weight.data, axis=1
        ).reshape(-1, 1)
        ...
        self.relationEmbeddingDict.weight.data /= torch.linalg.norm(
            self.relationEmbeddingDict.weight.data, axis=1
        ).reshape(-1, 1)
    ...
    def nf1Loss(self, input):
        ...
        margin = (torch.ones(d1.shape, requires_grad=False) * self.margin1).to(
            self.device
        )
        ...
        dst = torch.reshape(
            torch.linalg.norm(torch.maximum(euc + cr - dr - margin, zeros), axis=1),
            [-1, 1],
        )
        #  print(cr)
    ...
    def nf2Loss(self, input):
        ...
        dst = torch.reshape(
            torch.linalg.norm(torch.maximum(euc + newR - er - margin, zeros), axis=1),
            [-1, 1],
    ...
    def nf3Loss(self, input):
        ...
        # c1 input
        torch.cat((c1_o, r), dim=1)
        # c2 input
        torch.cat((c2_o, r), dim=1)

        # d1 input
        torch.cat((d1_o, r), dim=1)
        # d2 input
        torch.cat((d2_o, r), dim=1)
    ...
    def forward(self, input):
        ...
        loss1 = mseloss(
            loss1, torch.zeros(loss1.shape, requires_grad=False).to(self.device)
        )
        ...
        mseloss = nn.MSELoss(reduce=True)
        loss2 = mseloss(
            loss2, torch.zeros(loss2.shape, requires_grad=False).to(self.device)
        )
        ...
        mseloss = nn.MSELoss(reduce=True)
        loss3 = mseloss(
            loss3, torch.zeros(loss3.shape, requires_grad=False).to(self.device)
        )
        ...
        if len(input["nf4"]) == 0:
            ...
            mseloss = nn.MSELoss(reduce=True)
            loss4 = mseloss(
                loss4, torch.zeros(loss4.shape, requires_grad=False).to(self.device)
            )
        ...
        if len(input["disjoint"]) == 0:
            ...
            mseloss = nn.MSELoss(reduce=True)
            disJointLoss = mseloss(
                disJointLoss,
                torch.zeros(disJointLoss.shape, requires_grad=False).to(self.device),
            )
        ...
        if len(input["nf3_neg0"]) == 0:
            ...
            negLoss = mseloss(
                negLoss,
                torch.ones(negLoss.shape, requires_grad=False).to(self.device) * 2,
            )

        if self.vis_loss:
            vis_loss = torch.relu(
                0.2 - torch.abs(self.classEmbeddingDict.weight[:, self.embedding_dim :])
            )
            vis_loss = vis_loss.mean()
        else:
            vis_loss = 0

        totalLoss = [loss1 + loss2 + disJointLoss + loss3 + loss4 + negLoss + vis_loss]
    ...
    def save(self, folder, best=False):
        ...
        np.save(
            ...
            self.classEmbeddingDict.weight.detach().cpu().numpy(),
        )
        np.save(
            f"{folder}/relations{suffix}.npy",
            self.relationEmbeddingDict.weight.detach().cpu().numpy(),
        )
